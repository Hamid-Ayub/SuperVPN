...
public class IPRangeSet implements Iterable<IPRange> {
    ...
    public static IPRangeSet fromString(String ranges) {
        ...
        if (ranges != null) {
            for (String range : ranges.split("\\s+")) {
                ...
                } catch (Exception unused) {    /* besides due to invalid strings exceptions might get thrown if the string
    ...
    @Override
    public String toString() {    /* we could use TextUtils, but that causes the unit tests to fail */
