    ...
    private boolean checkPowerWhitelist() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (!pm.isIgnoringBatteryOptimizations(this.getPackageName()) &&
                ...
                if (getSupportFragmentManager().isStateSaved()) {    /* we might get called via service connection and manual onActivityResult()
        ...
        switch (requestCode) {
            case PREPARE_VPN_SERVICE:
                ...
                if (resultCode == RESULT_OK && mProfileInfo != null) {
                    ...
                    if (getSupportFragmentManager().isStateSaved()) {    /* onActivityResult() might be called when we aren't active anymore e.g. if the
